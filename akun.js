...
function verifyPin() {
    ...
    if (account.pin === '451') {
        ...
        try {
            ...
            // Gagal menyalin, tampilkan tombol salin manual
            showNotification(`Gagal menyalin OTP ke clipboard. Silakan salin OTP berikut secara manual: ${otp}`, true);
        }
    } else {
        showNotification('PIN salah!');
    }
...
function showNotification(message, showCopyButton = false) {
    const notification = document.getElementById('notification');
    notification.innerHTML = message;

    // Jika tombol salin diperlukan
    if (showCopyButton) {
        const copyButton = document.createElement('button');
        copyButton.textContent = 'Salin OTP';
        copyButton.onclick = function() {
            copyToClipboardAndContinue(otpStore[currentUsername].otp);
        };
...
function copyToClipboardAndContinue(otp) {
    navigator.clipboard.writeText(otp).then(() => {
        showNotification('OTP berhasil disalin ke clipboard.');

        // Melanjutkan proses setelah OTP disalin
        document.getElementById('otpMessage').textContent = `OTP untuk ${currentUsername} adalah: ${otp}`;
        document.getElementById('otpPopup').style.display = 'block';
        closePinPopup();
    }).catch(err => {
        showNotification('Gagal menyalin OTP lagi. Silakan coba lagi.');
    });
}
